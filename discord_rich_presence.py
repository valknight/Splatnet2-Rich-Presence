...
def get_minutes_since():
	...
	last_match.close()
	try:
		match_end = int(matches[0]["start_time"] + matches[0]["elapsed_time"])  # adds the seconds of the match to the unix time of the match starting
	except KeyError:
	...
	while True:  # The presence will stay on as long as the program is running
		for i in range(0, 4):
			...
			if minutes_since < timeout_minutes:
				RPC.update(details = details, state = state, large_image = "default", large_text = "Last match was {}, {}".format(last_match["game_mode"]["name"], last_match["rule"]["name"]))
